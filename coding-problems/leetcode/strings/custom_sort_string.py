...
should be "c", "b", and "a". 
Since "d" does not appear in S, it can be at any position in 
T. "dcba", "cdba", "cbda" are also valid outputs.
...
class Solution:
    def customSortString(self, S: str, T: str) -> str:
        res = ""
        # Iterate through sorted S, extracting order from T
        for c in S:
            if c in list(set(T)):
                res += (c * T.count(c))
                T = T.replace(c, "")
        # Append remaining chars in T
        return res + T
